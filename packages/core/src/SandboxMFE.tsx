 ...
 */
const loadAndInitRemoteContainer = (
	appName: string,
	url: string,
	...
	return new Promise((resolve, reject) => {
		...
		if (typeof __webpack_init_sharing__ !== "function") {
			return reject(
				new Error(
					"[SandboxMFE] Webpack `__webpack_init_sharing__` is not found on window.",
				),
			);
		}
		...
		script.onload = () => {
			...
			if (!container || typeof container.init !== "function") {
				return reject(
					new Error(`[SandboxMFE] Container "${appName}" not found on window.`),
				);
			}
	...
	return new Promise((resolve, reject) => {
		...
		const checkInterval = setInterval(() => {
			...
			if (Date.now() - startTime > timeout) {
				...
				reject(
					new Error(
						`[SandboxMFE] Shared dependencies ${deps.join(", ")} timed out`,
					),
				);
				return;
			}
			...
			const allLoaded = deps.every((dep) => {
				...
				return (
					sharedModule &&
					sharedModule.get &&
					typeof sharedModule.get === "function"
				);
			});
		...
		const loadAndRenderApp = async () => {
			try {
				...
				>((resolve, reject) => {
					...
					global.__ON_FACTORY_LOADED__ = (
						f: () => { default: React.ComponentType },
					) => {
